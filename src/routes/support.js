...
router.get('/support-tickets', authRequired, async (req, res) => {
  try {
    const [rows] = await pool.query(req.user.role === 'admin'
      ...
      : 'SELECT t.*, e.matricule FROM support_tickets t JOIN employees e ON e.id = t.employee_id WHERE e.matricule = ? ORDER BY t.created_at DESC', req.user.role === 'admin' ? [] : [req.user.matricule]);
    return res.json(rows.map((ticket) => ({ id: String(ticket.id), category: ticket.category, subject: ticket.subject, status: ticket.status, createdAt: ticket.created_at, messages: [] })));
  } catch (error) {
...
router.get('/support-tickets/:id', authRequired, async (req, res) => {
  try {
    const [ticketRows] = await pool.query('SELECT t.*, e.matricule FROM support_tickets t JOIN employees e ON e.id = t.employee_id WHERE t.id = ?', [req.params.id]);
    ...
    return res.json({ id: String(ticket.id), category: ticket.category, subject: ticket.subject, status: ticket.status, createdAt: ticket.created_at, messages: messageRows.map((message) => ({ id: String(message.id), sender: message.sender, content: message.content, createdAt: message.created_at })) });
  } catch (error) {
...
router.post('/support-tickets/:id/replies', authRequired, async (req, res) => {
  ...
  try {
    const [ticketRows] = await pool.query('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!ticketRows.length) return res.status(404).json({ message: 'Support ticket not found' });
    await pool.query('INSERT INTO support_messages (ticket_id, sender, content) VALUES (?, ?, ?)', [req.params.id, 'employee', message]);
    await pool.query('UPDATE support_tickets SET status = ? WHERE id = ?', ['in_progress', req.params.id]);
